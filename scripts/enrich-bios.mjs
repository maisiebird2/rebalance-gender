...
// ------------------------------------------------------------
// Decode a handful of common HTML entities that show up in
// og:description content (full entity decoding isn't needed for
// short bio snippets).
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// SoundCloud falls back to a generic, templated description when an
// account has no custom bio set, e.g.:
//   "Listen to 3LNA | SoundCloud is an audio platform that lets you
//    listen to what you love and share the sounds you create.."
...
// (?!@) prevents email addresses like mgmt@agency.com, contact@…, booking@…
// from being mistaken for label lines.
...
// Returns true if the text contains actual contact info (email, URL,
// or phone number), not just descriptor words like "REQUESTS".
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (category) {
      ...
      if (inlineValue && hasContactInfo(inlineValue)) {
        ...
        // Bio context resumes on the next line.
        ...
        // Style 3: "BOOKING REQUESTS" / "MANAGEMENT / GENERAL REQUESTS"
        // The inline text isn't contact info — it's a section-header
        // descriptor. Switch to section mode so subsequent lines
        // accumulate into this section until the next label.
        section = category;
      }
      continue;
    }
    ...
    // Non-label line.
    if (section !== "bio") {
      // We're inside an all-caps section block.
      info[section].push(line);
    } else if (EMAIL_ONLY_REGEX.test(line)) {
      // A bare email address in bio context (no keyword prefix) is
      // contact info — SoundCloud bios often put agent emails at the
      // very top without a label.
...
// ------------------------------------------------------------
// gate.sc is a link-click tracker SoundCloud wraps external URLs in,
// e.g. https://gate.sc?url=https%3A%2F%2Flinktr.ee%2Famelie.lens&token=...
// We decode these before running extractLinktree so Linktree URLs
// hidden inside gate.sc redirects are still found.
// ------------------------------------------------------------
function decodeGateSc(text) {
  return text.replace(
