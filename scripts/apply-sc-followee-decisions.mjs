...
// --- minimal ODS reader: unzip content.xml, walk rows/cells of the sheet ---
...
function readOdsRows(odsPath) {
  ...
  const sheetXml = sheets.get(SHEET_NAME) ?? (sheets.size === 1 ? [...sheets.values()][0] : null);
  if (sheetXml == null)
    throw new Error(
  ...
  const required = ["artist_id", "decision"];
  const missing = required.filter((c) => !header.map((h) => h.trim()).includes(c));
  if (missing.length)
    throw new Error(`Sheet is missing required column(s): ${missing.join(", ")} — header is: ${header.join(" | ")}`);
