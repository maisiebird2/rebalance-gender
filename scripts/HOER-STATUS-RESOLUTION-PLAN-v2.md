# HÖR pending-status resolution — implementation plan (v2)
...
Status: **plan only — resolver scripts not written yet.** The one prerequisite
schema change (the `name_search` punctuation strip) is **already implemented**,
so `name_search` is now a clean normalized key. When we start building the
scripts, open a new branch off `main` (project rule). These scripts hit
Supabase, so they can't be run or verified in the Cowork sandbox — they'll be
handed to Maisie to run locally.
...
- `name_search` strips both spaces **and** punctuation (diacritics folded,
  lowercased), so it is a valid exact-dup key. The Node normalizer is defined to
  mirror the column exactly so DB and script agree.
- **Match target — the "directory candidate set":** both the exact- and
  inferred-duplicate rules match HÖR artists against DB artists whose
  `directory_status` is `approved` **or** `sc_followee` (non-deleted).
  `sc_followee` is included deliberately — many SoundCloud-followee artists
  already in the DB overlap with the HÖR set, and we want those caught. The
  pending HÖR batch itself is naturally excluded by this status filter (it's
  `pending`), so no separate HÖR-link exclusion is needed.
- The trigram index on `name_search` is **partial to `approved` only**, so it
  does not cover the `sc_followee` candidates. Fuzzy lookup therefore computes
  similarity in Node over the fetched candidate set — fine, since these run
  locally against a paginated full read.
...
- Compare `normalizeName(hoer.name)` against the `name_search` of every artist
  in the **directory candidate set** (`directory_status` ∈ {`approved`,
  `sc_followee`}, non-deleted). Because `name_search` is the aligned normalized
...
1. **Candidate shortlist:** directory-candidate-set artists (`directory_status`
   ∈ {`approved`, `sc_followee`}, non-deleted) whose normalized name is *similar*
   (trigram / normalized-Levenshtein above a threshold — start ~0.55, tune).
   Keeps the expensive comparison to a small subset.
